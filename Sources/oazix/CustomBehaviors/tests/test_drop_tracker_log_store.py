...
def test_parse_old_csv_format_back_compat():
    csv_text = "\n".join(
        [
            ...
            "2026-02-22 12:00:00,BotA,55,Player Three,Holy Staff,1,White",
        ]
    )
    ...
    assert row.player_name == "Player Three"
...
def test_parse_headerless_rows_include_first_and_following_rows():
    csv_text = "\n".join(
        [
            "2026-02-22 12:00:03,BotA,248,Temple of the Ages,Player Three,Holy Staff,1,White,ev-hl-1,\"line\",489,mesmer@test",
            "2026-02-22 12:00:04,BotA,248,Temple of the Ages,Player Three,Holy Staff,2,White,ev-hl-2,\"line\",490,mesmer@test",
        ]
    )
...
def test_parse_old_csv_invalid_map_id_uses_unknown_fallback():
    csv_text = "\n".join(
        [
            "Timestamp,ViewerBot,MapID,Player,ItemName,Quantity,Rarity",
            "2026-02-22 12:00:05,BotA,not-a-map,Player Three,Holy Staff,1,White",
        ]
    )
...
def test_log_store_private_helper_edges():
    ...
    assert log_store._infer_has_map_name(list("not-a-list")) is True
...
def test_parse_new_csv_format_with_event_fields():
    csv_text = "\n".join(
        [
            ...
            "2026-02-22 12:00:01,BotA,248,Temple of the Ages,Player Three,Holy Staff,1,White,85e699300008,\"Holy Staff\\nValue: 224 gold\",487",
        ]
    )
    rows = parse_drop_log_text(csv_text, map_name_resolver=lambda _: "unused")
    assert len(rows) == 1
...
def test_parse_new_csv_format_with_sender_email():
    csv_text = "\n".join(
        [
            ...
            "2026-02-22 12:00:02,BotA,248,Temple of the Ages,Player Three,Holy Staff,1,White,85e699300009,\"line\",488,mesmer@test",
        ]
    )
    rows = parse_drop_log_text(csv_text, map_name_resolver=lambda _: "unused")
    assert len(rows) == 1
...
def test_parse_headerless_new_format_row():
    csv_text = "2026-02-22 12:00:03,BotA,248,Temple of the Ages,Player Three,Holy Staff,1,White,ev-hl,\"line\",489,mesmer@test\n"
    rows = parse_drop_log_text(csv_text, map_name_resolver=lambda _: "unused")
    assert len(rows) == 1
    row = rows[0]
    assert row.player_name == "Player Three"
...
def test_append_drop_log_rows_upgrades_legacy_header_and_preserves_existing_fields():
    ...
    legacy_text = "\n".join(
        [
            "Timestamp,ViewerBot,MapID,MapName,Player,ItemName,Quantity,Rarity",
            "2026-02-22 12:00:02,BotA,248,Temple of the Ages,Player Three,Holy Staff,1,White,ev-old,\"line-old\",488,mesmer@test",
        ]
    ) + "\n"
...
def test_replace_drop_log_row_updates_existing_event_row():
    temp_dir = _make_local_temp_dir()
    file_path = temp_dir / "replace_row.csv"
    original = DropLogRow(
        timestamp="2026-02-22 13:20:00",
        viewer_bot="BotA",
        map_id=248,
        map_name="Temple of the Ages",
        player_name="Player Three",
    ...
    updated = DropLogRow(
        ...
        map_name="Temple of the Ages",
        player_name="Player Three",
        item_name="Furious War Axe of Warding",
        quantity=1,
        rarity="Gold",
        event_id="ev-replace",
        item_stats="Furious War Axe of Warding\nDamage: 6-28",
        item_id=77,
        sender_email="tri@test",
    )
    try:
        append_drop_log_rows(str(file_path), [original])
        replaced = replace_drop_log_row(str(file_path), updated)
        parsed = parse_drop_log_file(str(file_path))
        assert replaced == 1
        assert len(parsed) == 1
